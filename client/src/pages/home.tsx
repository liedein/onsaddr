...
export default function Home() {
  ...
  const incrementUsage = () => {
    ...
    console.log("조회 횟수 증가 완료"); // 확인용 로그
  };
      ...
      <header className="bg-gray-800 shadow-lg border-b border-gray-700">
        <div className="flex w-full items-center px-4 py-4">
          ...
          <div className="w-12 flex justify-end">
            <button onClick={handleRefresh} className="p-2 text-gray-400 hover:text-gray-100 hover:bg-gray-700 rounded-lg">
      ...
      <main className="flex-1 flex flex-col relative">
        ...
        <div className="bg-gray-800 border-t border-gray-700 pt-5 pb-4 px-2 flex flex-col space-y-3">
          ...
          <div className="flex items-start">
            ...
            <textarea maxLength={100} rows={2} className="text-base bg-gray-100 text-gray-900 px-3 py-2 rounded-md flex-1 resize-none" value={detail} onChange={e => setDetail(e.target.value)} placeholder="세부내역 입력" />
          </div>

          {/* 카운트 표시 */}
          <div className="mt-3 text-center border-t border-gray-700 pt-3">
            <span className="text-sm text-gray-400">오늘 기기 조회 횟수 : </span>
            <span className="text-base text-emerald-400 font-bold">{usageCount}</span>
            <span className="text-sm text-gray-400"> / {USAGE_LIMIT}</span>
          </div>
        </div>
      </main>
